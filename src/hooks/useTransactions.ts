  ...
  return useMutation({
    mutationFn: async (id: string) => {
      const { data: tx, error: fetchError } = await supabase
        .from("transactions")
        .select("id,category,is_auto_generated,notes")
        .eq("id", id)
        .maybeSingle();

      if (fetchError) throw fetchError;

      // Salary entries generated by schedule should be "skipped" for this period,
      // not hard-deleted, so they do not reappear on refresh.
      if (tx?.category === "salary" && tx.is_auto_generated) {
        const existingNotes = (tx.notes || "").trim();
        const voidedAt = `voided_at=${new Date().toISOString()}`;
        const notes = existingNotes ? `${existingNotes};${voidedAt}` : voidedAt;

        const { error: voidError } = await supabase
          .from("transactions")
          .update({ status: "void", is_paid: false, notes })
          .eq("id", id);
